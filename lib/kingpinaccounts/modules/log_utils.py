...
import inspect
from kingpinaccounts.modules.control import transPath, setting as getSetting, lang, joinPath, existsPath
...
DEBUGPREFIX = '[COLOR red][ Kingpin Accounts %s ][/COLOR]'
...
def log(msg, caller=None, level=LOGINFO):
	...
	try:
		...
		if debug_location == '1':
			log_file = joinPath(LOGPATH, 'kingpinaccounts.log')
		...
		traceback.print_exc()
		import xbmc
		xbmc.log('[ script.module.kingpinaccounts ] log_utils.log() Logging Failure: %s' % (e), LOGERROR)
...
def error(message=None, exception=True):
	try:
		...
		if exception:
			...
			addon = 'script.module.kingpinaccounts'
		...
		import xbmc
		xbmc.log('[ script.module.kingpinaccounts ] log_utils.error() Logging Failure: %s' % (e), LOGERROR)

def clear_logFile():
	cleared = False
	try:
		from kingpinaccounts.modules.control import yesnoDialog
		...
		import xbmc
		xbmc.log('[ script.module.kingpinaccounts ] log_utils.clear_logFile() Failure: %s' % (e), LOGERROR)
		cleared = False
	return cleared
...
def view_LogFile(name):
	try:
		...
		from kingpinaccounts.modules.control import addonPath
		log_file = joinPath(LOGPATH, '%s.log' % name.lower())
		if not existsPath(log_file):
			from kingpinaccounts.modules.control import notification
...
def upload_LogFile():
	...
	try:
		import requests
		from kingpinaccounts.modules.control import addonVersion, selectDialog
		f = open(log_file, 'r', encoding='utf-8', errors='ignore')
		text = f.read()
		f.close()
		UserAgent = 'kingpinaccounts %s' % addonVersion()
		...
		if 'key' in response.json():
			...
			log('Kingpin Accounts log file uploaded to: %s' % result)
			...
			notification(message='kingpinaccounts Log upload failed: %s' % str(response.json()['message']))
			log('Kingpin Accounts Log upload failed: %s' % str(response.json()['message']), level=LOGERROR)
		else:
			notification(message='kingpinaccounts Log upload failed')
			log('Kingpin Accounts Log upload failed: %s' % response.text, level=LOGERROR)
	except:
		error('Kingpin Accounts log upload failed')
