...
class AddonCheckUpdate:
	def run(self):
		xbmc.log('[ script.module.kingpinaccounts ]  Addon checking available updates', LOGINFO)
		try:
			import re
			import requests
			repo_xml = requests.get('https://raw.githubusercontent.com/mr-kodi/repository.kingpinaccounts/master/zips/addons.xml')
			if repo_xml.status_code != 200:
				return xbmc.log('[ script.module.kingpinaccounts ]  Could not connect to remote repo XML: status code = %s' % repo_xml.status_code, LOGINFO)
			repo_version = re.search(r'<addon id=\"script.module.kingpinaccounts\".*version=\"(\d*.\d*.\d*)\"', repo_xml.text, re.I).group(1)
			...
			if check_version_numbers(local_version, repo_version):
				while control.condVisibility('Library.IsScanningVideo'):
					control.sleep(10000)
				xbmc.log('[ script.module.kingpinaccounts ]  A newer version is available. Installed Version: v%s, Repo Version: v%s' % (local_version, repo_version), LOGINFO)
				control.notification(message=control.lang(32072) % repo_version, time=5000)
			return xbmc.log('[ script.module.kingpinaccounts ]  Addon update check complete', LOGINFO)
		except:
			import traceback
			traceback.print_exc()
...
class PremAccntNotification:
	def run(self):
		...
		xbmc.log('[ script.module.kingpinaccounts ]  Debrid Account Expiry Notification Service Starting...', LOGINFO)
...
if control.setting('checkAddonUpdates') == 'true':
	...
	xbmc.log('[ script.module.kingpinaccounts ]  Addon update check complete', LOGINFO)

PremAccntNotification().run()

xbmc.log('[ script.module.kingpinaccounts ]  service stopped', LOGINFO)